...
export function SessionItem(props: Props) {
  ...
  const handleDeleteFromMenu = (e: React.MouseEvent) => {
    e.stopPropagation();
    setContextMenu(null);
    onDelete();
  };
  ...
  return (
    <>
      ...
      {contextMenu && (
        <div
          className="fixed z-[100] min-w-36 rounded-md border border-zinc-700 bg-zinc-900 py-1 text-xs text-zinc-300 shadow-xl"
          style={{ left: contextMenu.x, top: contextMenu.y }}
          onClick={(e) => e.stopPropagation()}
        >
          <button
            className="w-full px-3 py-1.5 text-left hover:bg-zinc-700/70 hover:text-zinc-100"
            onClick={handleRenameFromMenu}
          >
            Rename
          </button>
          <button
            className="w-full px-3 py-1.5 text-left text-red-300 hover:bg-red-500/15 hover:text-red-200"
